...
- **Default target: the app's home/entry page.** The typical host is a **React (Vite/TypeScript) application** — resolve the root route's component (e.g. `App.tsx`, or the `/` route element when a router is present) and replace its existing hero (or its top-of-page block) with the `<HeroScroll>` journey, honoring the reference's sticky constraint (no transformed/overflow-clipped ancestors). For non-React hosts, the entry `index.html`. Slot 8 governs everything below the journey: with `sweep: page` the below-journey content is redesigned to the film's brand and exit tone (an unqualified `/bt-hero` therefore redesigns the whole landing page); with `sweep: hero` nothing below the journey is touched beyond the handoff seam.
