...
| **`unity-level`** | a Unity scene via the Unity CLI — terrain, light rig, probes, bake settings, volumes | the **exported** level served over the toolkit dev server, in a browser, plus the exported scene metadata | `references/unity-level-pipeline.md` |
| **`blender-model`** | a `.blend`/FBX via headless Blender `bpy` — geometry, UVs, PBR maps, LODs, skinning | the asset **loaded in BabylonJS** in a browser under a frozen lighting rig | `references/blender-model-pipeline.md` |

**Read the matching pipeline reference before round 1**, and again on every `--resume` (a resumed session has lost that context). Each one defines its prerequisite gate, build surface, part taxonomy, verify recipe, extra critic gates and known failure modes. A job may span kinds — a `unity-level` job routinely sends model work through the Blender pipeline — in which case read both.
...
| `references/target-images.md` | generating, prompting, reachability-checking, locking the target; locked cameras |
| `references/loop-templates.md` | Template A and Template B loop cards |
| `references/unity-level-pipeline.md` | the Unity CLI build + verify pipeline |
| `references/blender-model-pipeline.md` | the Blender headless CLI build + verify pipeline |
...
> **Direction matters.** The doctrine above governs *consuming* supplied prefabs. A `unity-level` job is usually **authoring** the thing that carries the metadata — there, the equivalent discipline is honoring the exporter's contract (see the pipeline reference's export-boundary gate), not preserving components you are in the act of creating. Say which situation applies in the loop card so critics do not misapply it.
...
3. **THE TARGET + CAMERAS** — Per `references/target-images.md`: does the user have reference media, is there an existing artifact to refine, or do we generate the target? Agree the target image(s), run the **reachability pass** (Reachable / Approximable / **OUT OF REACH**), and agree the **locked cameras** — one per target, three to five, including a detail crop. Plus numeric benchmarks: target FPS @ resolution, max load time, draw-call / texture / triangle budgets. If the user has no benchmark, making a defensible one is part of the interview, not skipped.
...
6. **BOUNDARIES** — Time/cost expressed in **rounds and attempts, not wall-clock** (wall-clock cannot survive a two-week pause; counters can): max total rounds, max attempts per part before a forced strategy change, max consecutive no-improvement rounds. Permission gates: no deploy, no purchases/paid services, no credentials, no destructive ops, no contacting anyone — without explicit approval. For `unity-level`: the **bake tier policy** (how many rounds at preview tier before a production bake). Escalation rules: what blocks and waits for the user.
7. **Kind specifics** — For `web-game`: genre & camera, physics (Havok default), input, audio, asset pipeline, scene scale, target browsers, and explicit scope cuts. For `unity-level`: which scene, render pipeline, terrain in scope?, existing lighting to preserve, copilot vs headless Editor. For `blender-model`: which asset, glb-direct or via-unity, in-place or new path, tri budget, texel density, rig to match.
...
├── pipeline.md       # pipeline state: project paths, licence tier, dev server, bake tier
...
4. **Verify with the real artifact** — run the pipeline's verify recipe and capture evidence **from the locked cameras** into `_gauntlet/<name>/evidence/`: screenshots, metrics (FPS, draws, load time, console errors), and the pipeline's own read-back (exported scene metadata, GUID check, tri count). **Screenshots are required for visual parts.** If the host cannot produce the evidence, mark the part `unverified` — it can NEVER be flipped to `- [x]` on the builder's word.
...
**If the user attaches nothing**, the interview generates the target instead — a real in-engine screenshot, never concept art (`references/target-images.md` § 2):
...
**Interview settles:** kind `unity-level`; three locked cameras (`gauntlet_hero` down the alley, `gauntlet_side` across the puddles, `gauntlet_detail` on a sign); pass threshold 8.0; budgets 60 FPS @1080p, ≤900 draws; bake policy *preview until round 8, then production*.
...
unity command bt_export_level --scene Assets/Scenes/NeonAlley.unity --project-path "$PROJ" --timeout 900
unity command bt_devserver_start --project-path "$PROJ"
# browser: select gauntlet_hero, screenshot -> evidence/round-NN-p7-hero.png
jq '.scenes[0].extras.metadata | {license,skybox,ambientlighting,reflectionprobeintensity,fogmode,exposure,tonemapping}' \
   "$PROJ/Export/scenes/neonalley.gltf" > evidence/round-NN-p7-metadata.json
```

The critic scores hero + side + detail against their targets, returns the full gap list, and checks the export-boundary gate — *did `reflectionprobeintensity` actually change, or did the round only move a Unity slider that never crossed?*
...
5. Bake tiering matters more here than anywhere, because the scene is already heavy: preview tier for the convergence rounds, one production bake before the integration critic.

Because the scene was authored in the GUI it already has a LightingSettings asset — but **always pass `--scene`** anyway, or a fresh Editor session silently exports the template's default scene instead.

## Example 3 — A high-fidelity Blender model with spit and polish
...
Reference is the attached turntable.
```

**Interview settles:** kind `blender-model`; route `via-unity` (it needs components later, so Blender fixes the model and Unity composes); **write mode IN PLACE** so the GUID and importer settings survive every round; a frozen viewer rig (one HDRI, one exposure, no post) written into `pipeline.md`; four locked angles including a detail crop; budgets 45k tris, 512 px/m texel density.

**The two rules that protect the project**, both hard gates every round:
...
Without `--python-exit-code 1` a crashed script exits `0` and the loop records a failed round as a success. Without in-place writing, every round mints a new GUID and quietly detaches the model from every scene that used it.

**Evidence is the engine, not Cycles.** The `.glb`/level is re-exported and loaded in the browser under the frozen rig — a Cycles render flatters a model in ways a real-time renderer will not. Render the same angles from Blender as a *secondary* frame so the critic can say which side of the boundary a gap is on: Blender right + engine wrong means the material translation is the problem, not the sculpt.

**Textures are generated, never procedural stand-ins** — `generate_image` for albedo, normal and roughness detail. "Substituted procedural noise for the missing map" is a gate failure, and it is the single most common reason a model reads as plastic.
