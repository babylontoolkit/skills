---
name: bt-plan
description: "The Babylon Toolkit Plan Skill creates the detailed technical plan for the specified feature spec file. Use when asked to plan or produce implementation tasks for an existing spec. Also supports a Quick Plan mode: when given only a brief (no spec file), it interviews the user to build a mini-spec and plans from that. The plan is written to be decision-complete — a numbered Decisions log, a Design Reference with the real interfaces, names, values and patterns, and self-contained tasks — so that a long run of tasks, each executed in a fresh context days apart, stays cohesive and resumable. Always planning-only — it writes the plan file and stops; execution is the separate bt-execute skill."
allowed-tools: Read, Grep, Glob, Write, WebFetch(domain:raw.githubusercontent.com), Agent, Task
---

Create a detailed technical implementation plan for the specified feature spec and save it in the `_specs` folder as `<feature-name>_plan.md`. Always generate implementation tasks or steps (prefer to call them tasks).
...
> Quick Plan is still **PLANNING MODE**. It ends the moment `_specs/<feature-name>_plan.md` is finalized. Do **NOT** implement any task, do **NOT** edit application/source files, do **NOT** run builds or tests, and do **NOT** continue into the tasks you just wrote — not even the first one, not even if the tasks look small, obvious, or trivially automatable. The user executes tasks separately with the **bt-execute** skill, deliberately, with a clean context between steps. Auto-executing destroys that workflow.
...
| `Decisions` section | Start empty; every interview answer and every choice you make becomes a plan `## Decisions` entry. |

**The one and only difference in Quick Plan mode is operating without a spec file** — the interview produces a mini-spec that stands in for it. Everything else is identical to a normal run: Step 1's comprehensive codebase analysis (including reading root `SPEC.md`) runs in full, the plan document has the same structure and the same decision-completeness bar, the cold-context audit in Step 3 still runs, the SPEC.md write-back task is still appended when the inferred `spec_impact` is `yes`, and the run still **STOPS after writing the plan file**. Nothing about Quick Plan grants permission to implement.
...
- If the user declines to answer or says "just make the plan", proceed with the defaults and state which assumptions you made in the plan's Codebase Analysis section **and** record each one as a `## Decisions` entry, so every later task sees the same assumption.
...
Downgrading is not a neutral choice. It removes the one property that makes the step worth running: independence. An auditor that is the same context which just wrote the plan cannot adversarially check it — it re-confirms its own reasoning and reports "no gaps".
...
5. **Mark the phase transitions.** Emit a line when research starts, when it completes, when drafting begins, when the cold-context audit starts and what it found, and when the final write happens — e.g. `🧩 [bt-plan] Research complete — <n> findings synthesized` followed by a **3–6 bullet research summary** (the key findings, with file paths) so the user can see what the run is based on, then `✍️ [bt-plan] Drafting the document …`.
...
7. **A silent gap may never span more than a single tool call.** If one tool call is itself long (a subagent, a large fetch, a large write), the line *preceding* it must say what it is and roughly how long it is expected to take. A plan with 50 tasks is a large write — say so before writing it, and consider writing it in two or three appends with a status line between them.

This applies to every phase of this skill — the Agent Reference fetch, `SPEC.md`/`DESIGN.md` reads, the fan-out research, sibling-skill reads, interviewing, drafting, and the audit. When in doubt, say what you are doing.
...
**Do the API homework once, here.** When a task uses a Babylon Toolkit component or BabylonJS API, the plan should name the exact class/method/property and show the call the way the Agent Reference (or its sub-document) shows it, in the Design Reference › *API usage* section — so every task across the run uses the same API the same way instead of each fresh context rediscovering it. Fetch the relevant sub-documents *now* and copy the correct usage into the plan.
...
This command runs in PLANNING MODE. Research read-only and produce ONLY the technical plan document. Do NOT implement the feature, edit any existing application/source files, or run build, test, or other shell commands. The only file you may create or edit is the plan markdown in `_specs/`, named `<feature-name>_plan.md`. (You may write it more than once — a draft, then the audited final version — but nothing else.)

**This applies identically in Quick Plan mode.** Planning and execution are deliberately separate skills: `bt-plan` writes the plan, `bt-execute` runs it — one task at a time, with a clean context between tasks, at the user's discretion. Rolling straight from planning into implementation defeats the entire purpose of that split. Finalizing the plan file is the LAST action of this skill in every mode.
...
- On the sequential path, one `🔎 [bt-plan] …` line before each read/search (each of the investigation points below) and one result line after it.
...
Before writing a single task, investigate the actual codebase read-only. This is mandatory — do NOT generate any plan content until this analysis is complete. Read and search the repo to discover, not assume:

1. Read the referenced feature spec in full (from `_specs/` or the file named in `arguments`), **and read the project `SPEC.md` at the repository root in full.** SPEC.md is the source of truth for the durable architecture, systems, conventions, and decisions — the plan MUST conform to it. Note the feature spec's `spec_impact` field, its `Project Spec Alignment` section, its `Decisions` section (carry every entry forward into the plan's `## Decisions`), and its `Open Questions` (each must be **resolved** in the plan — see Step 2). **In Quick Plan mode there is no feature spec file** — treat the brief as the feature request, still read root `SPEC.md` in full, and *infer* the `spec_impact` and Project Spec Alignment from the analysis per the Quick Plan defaults table. If the plan you are about to write would conflict with SPEC.md (contradict a decision, cross a system boundary, break a convention), STOP and flag the conflict to the user before writing the plan; do not silently override the project spec.
2. Map the project: top-level structure, entry points, how the app is built and run (build scripts, test runner, package manifests). Record the exact build and test commands — the plan's tasks will quote them.
3. Identify the conventions actually used in this repo: naming, file/folder organization, state management, styling, error handling, testing patterns. Capture each as a verbatim snippet with its `file:line`, not a description.
4. Find the closest existing feature(s) or modules to the one being planned and study how they are implemented — the plan should follow these patterns, and its tasks should point at the exact functions to mirror.
5. List the concrete integration points the feature will touch: files, modules, APIs, data models, routes, config — with the real signatures at each point and the line where new code will be inserted.
...
8. Establish the test baseline: how tests are run, which tests cover the code being touched, and whether the suite is currently green (record the failing set if not, so a task run days later can tell new failures from old ones).
9. If the feature builds on a **sibling-skill pattern** (e.g. bt-design's 3D-Hero-Scroll), read that sub-skill's reference **and its templates**. *(How to read one: where skills are loaded with a tool — the Babylon Toolkit App Builder platform — call `load_skill('<name>')`, then fetch its references and templates with `read_skill_resource` using the paths the load returns, never a guessed path. Where skills are files on disk — Claude Code — read them from the same skills directory, `~/.claude/skills/` or the project's `.claude/skills/`. Skip the load for anything already in your context.)* The plan MUST **copy and configure the sub-skill's template engine — never re-implement it from memory**; re-implementation is exactly how documented behaviors (`sweep`, veiled cuts, the preload gate, HUD auto-hide, graceful degradation) silently get dropped. Turn every behavioral-config requirement the spec records (e.g. `sweep: page`) into a concrete task whose Acceptance asserts that behavior in observable terms, and write the exact config values into the Design Reference.

If the spec or codebase is too ambiguous to analyze responsibly, stop and ask the user rather than guessing. **Ask now, while the whole feature is in view** — a question deferred to execution gets answered by whichever task happens to hit it, in isolation, without the context to answer it consistently.

## Step 2. Write the plan

Emit `✍️ [bt-plan] Writing draft _specs/<feature-name>_plan.md …` before writing, and `💾 [bt-plan] Draft written — _specs/<feature-name>_plan.md (<n> tasks)` once the file is saved.

Only after Step 1 is complete, write the plan markdown to `_specs/` as `<feature-name>_plan.md`. The document has **five required sections, in this order**. A plan missing any of them is invalid; do not produce one.

### 2.1 `## Codebase Analysis`

Summarize the findings from Step 1 (cite the real files/modules you inspected, with `file:line`) — this is the evidence that the analysis happened. Include:

- a short **SPEC.md alignment** note: which SPEC.md sections the plan conforms to, and whether the feature is spec-impacting (carry over the feature spec's `spec_impact`, or in Quick Plan mode the value you inferred, and say it was inferred);
- the **test baseline** (command, current pass/fail state, which failures are pre-existing);
- in Quick Plan mode, the **interview answers** that stand in for the spec.

### 2.2 `## Decisions`

The feature-scoped decision log — the part of the plan that keeps task 40 consistent with task 3. One entry per decision, numbered `D1`, `D2`, … so tasks can cite them. Carry every entry from the feature spec's `Decisions` section forward (keep its wording, add the D-number), then add every decision *you* made while planning — architecture, layout, naming, algorithms, error policy, test strategy — using the table in *The plan is the memory* above as the checklist. Every entry has the same shape:
...
The ordered checklist. Use GitHub-style checkboxes so progress can be tracked directly in the file — one task per checkbox, numbered `T1`, `T2`, `T3` … in dependency order, each small enough to be implemented and verified on its own in a fresh context (see *many small, self-contained tasks* above). Every task uses this block. The fields marked **required** must be present on every task; the others are omitted only when they genuinely have nothing to say.
...
- **Acceptance is observable.** Something a verifier can check by running a command or inspecting a file: a test count, a grep result, a rendered behaviour ("PLAY auto-scrolls to the document bottom"), never "works correctly".
...
- [ ] **T<n>** — Update SPEC.md to match what was built
  ...
  - Applies: D1–D<k>
  - Steps:
    1. Under `## Game Systems` › `<System>`, replace the paragraph beginning "<first words>" with the text in **Code** › *Game Systems*.
    2. Under `## Conventions`, append the bullet(s) in **Code** › *Conventions*.
    3. Under `## Decisions`, append (never edit or delete) the entries in **Code** › *Decisions*, newest last.
    4. Under `## Dependencies`, add <dependency + version + why> — or, if none was added, make no change.
  - Code:
    <the exact markdown for each section>
  - Acceptance: SPEC.md accurately describes the architecture/systems/conventions/dependencies as actually implemented by the tasks above; no section contradicts the shipped code; the Decisions entries are appended, nothing deleted; new dependencies are listed. Follow SPEC.md's "How to update this spec" contract: **replace/merge** current-state sections (removing seed placeholders on first real content), **append** to the Decisions log.
  - Verify: `grep -n "<a distinctive phrase from each inserted paragraph>" SPEC.md` → one hit each.
```
...
### 2.5 `## How to execute this plan`

Include this exact section verbatim in the document so the plan is self-describing no matter how it is later run:
...
## Step 3. Cold-context audit (REQUIRED before the plan is final)
...
Emit `🧾 [bt-plan] Audit found <n> gaps across <m> tasks` (or `… found NO GAPS`). Then **resolve every reported gap by editing the plan** — add the missing decision to `## Decisions`, the missing code/signature/name to `## Design Reference`, the missing instruction to the task — and emit `🔧 [bt-plan] Resolved <n>/<n> gaps`. If the audit found more than a handful of gaps, run it **once more** on the revised plan (`🕵️ [bt-plan] Second audit pass …`); stop after the second pass regardless, and list any gap you chose not to resolve, with why, in the plan's Codebase Analysis. A gap the audit reports that you cannot resolve without the user is a question for the user — ask it now.

## Step 4. Finalize and stop

Emit `💾 [bt-plan] Plan finalized — _specs/<feature-name>_plan.md (<n> tasks, <k> decisions)`.

Your final response is a short summary: the number of tasks and decisions, the two or three most consequential decisions (with their D-numbers), anything you were unable to resolve, and the exact next-step hint:
...
Then **STOP**. Do not begin T1. Do not offer to start.
