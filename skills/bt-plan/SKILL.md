...
3. Size the research: `small` → inline; `medium` / `large` (or no Research Notes) → up to 3 **read-only** subagents returning findings and paths, not file dumps.
...
6. **No ceremony.** No standalone "verification sweep / open the review / final check" tasks, and no ledgers, md5 or environment records, archives, checked-in capture trees, standing evidence suites, doc-integrity registration or staged promotions — unless the brief asks.
7. **No brittle pins.** Never pin counts later tasks will change (tests passed, suite sizes, hashes). Say "the named tests pass, no new failures versus the baseline".
...
**`## Estimated execution time`** — a table `Phase | Tasks | Build | Prove | Estimate` (Build = code and tests; Prove = exports, captures, comparisons, fix-until-green), then one **Total** range in hours (days if over ~16 h) = the sum of the phases plus 20–40 % for fix loops, the `--strict` multiplier (~2–3×), what would cut Prove if it exceeds Build, and the biggest uncertainty. Rates per phase (whole-phase figures; split each into Build and Prove by what the phase does): code + unit tests 15–25 min · adds a C#/native build or a full test chain 30–45 min · adds Unity authoring/bake/export or browser captures 45–90 min · a visual fix-until-green loop 1–3 h. An estimate, never a pin.
