...
**Backend mapping (Claude Code and other MCP hosts).** Where no platform-native `generate_*` is present, use the **KIE MCP** servers by default (`web-kie-servers.md`): pass `out_path` under the project's `assets/generated/` folder and reference that path. When **Higgsfield MCP** is the configured or requested backend (`web-higgsfield-mcp.md`), the call returns a job, not a path. Submit everything in one parallel round (`generate_image_batch`, plus at most one `generate_video`), write the files with their fallbacks in place, then `jobs_wait` / `job_status` and `curl` each result into `assets/generated/` under the names you already referenced. Never hot-link the Higgsfield CDN URL. Preflight with `get_cost: true` and always send `use_unlim: false`.
