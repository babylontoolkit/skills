...
**Backend mapping (Claude Code and other MCP hosts).** Where no platform-native `generate_*` is present, use the **KIE MCP** servers by default (`web-kie-servers.md`): pass `out_path` under the project's `assets/generated/` folder and reference that path. When the **Higgsfield CLI** is the set-up or requested backend (`web-higgsfield-cli.md`), generate through the reference's `scripts/hf-generate.mjs` wrapper, which saves each result to `--out` (for example `node scripts/hf-generate.mjs gpt_image_2_5 --prompt "…" --aspect_ratio 16:9 --resolution 2k --out assets/generated/hero-bg.png`). Launch all the calls in ONE parallel round in the background (shell `&`), write the files with their fallbacks in place, then `wait`. Image results are **PNG**, so reference `.png` paths (the wrapper saves `--out x.jpg` as `x.png` and reports it). Never reference a Higgsfield CDN URL. Run `--cost` first and state the total.
