...
**ENGINE CONTRACT — splash element IDs are LAW (STRONGLY ENFORCED, zero exceptions).** "Keep the MECHANICS" above includes the DOM identity of the splash skeleton: the engine shows/hides the splash screen and writes its label/status text by looking up these EXACT element ids/names — `xbabylonjsSplashScreen` (the main splash panel), `babylonjsLoadingDiv`, `babylonjsLoadingText`, `babylonjsLoadingDivStyle`, and `xbabylonjsStatusTextDiv`. If any of these is renamed, removed, or unmounted, the engine cannot hide the splash and the scene underneath is never revealed. Every splash redesign keeps all five present with ids byte-for-byte unchanged; creativity happens around them — restyle via CSS targeting the ids, nest new elements inside/beside them, animate them freely. `babylonjsLoadingText` and `xbabylonjsStatusTextDiv` receive engine-written text at runtime, so they must remain real text-bearing elements. Before finishing any splash redesign, verify all five ids still exist — a missing id means the redesign is broken, not done.

**IMPORTS ARE COPIED, NEVER RE-DERIVED (STRONGLY ENFORCED, zero exceptions).** The other half of "keep the MECHANICS": when you rewrite an existing file — `splash.tsx`, `overlay.tsx`, `loading.tsx`, any component already wired to the framework — READ it first and carry its **entire import block** across **unchanged, character for character**, along with every `GameManager`, `EventBus` and `useUnifiedNavigation` line. Replace markup, styles and copy; touch nothing above them. Do not retype an import from memory, do not "tidy" or reorder it, do not change a default import into a named one. A single altered import is a build error, and the file you broke is one the player sees before anything else. Concretely, the slip that has already shipped a project that would not compile: **`GameManager` is a DEFAULT export** — `import GameManager from '../babylon/globals'`, NEVER `import { GameManager } from '../babylon/globals'`. A framework's export shape is not something to infer from the name; it is something to copy from the file in front of you. SELF-CHECK before finishing: compare your new import block to the original's — any difference means fix it before you stop.
