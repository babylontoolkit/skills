...
const TARGETS = [
  ...
  },
  {
    ...
    default: true,
    global: { skills: null, instructions: '~/.codex/AGENTS.md', config: '~/.codex/config.toml' },
    project: { skills: null, instructions: 'AGENTS.md', config: '.codex/config.toml' },
  },
  {
    id: 'gemini',
  ...
  },
  {
    ...
    default: false,
    global: { skills: '~/.codex/skills', instructions: null, config: '~/.codex/config.toml' },
    project: { skills: '.codex/skills', instructions: null, config: '.codex/config.toml' },
  },
];
...
function resolveTargets({ mode = 'global', projectRoot = process.cwd(), ids = null, legacyCodex = false } = {}) {
  ...
  for (const t of selected) {
    ...
    const config = resolveTargetPath(spec.config, mode, projectRoot);
    if (config) {
      if (!configFiles.has(config)) configFiles.set(config, { file: config, targets: [] });
      configFiles.get(config).targets.push(t.id);
    }
